...
class BoursoramaApi {
   ...
   */
  constructor(clientNumber, password, downloadPath) {
  ...
  async getAccounts() {
    ...
    const accounts = await this.#page.$$eval(
      ".c-panel__item, .c-info-box__item",
      (els) =>
        els.map((e) => ({
          label: e.querySelector(".c-info-box__account-label")?.textContent,
          balance: e.querySelector(".c-info-box__account-balance--positive")
            ?.textContent,
          link: e
            .querySelector(".c-info-box__link-wrapper")
            ?.getAttribute("href"),
        }))
    );
  ...
  async getMovements(accountId, from, to) {
    await this.#page.goto(
      `https://clients.boursorama.com/compte/cav/${accountId}/mouvements`
    );
    ...
    let downloadedFile = Utils.waitForFile(
      { path: this.#downloadPath },
      (evt, fileName) => fileName.endsWith(".csv")
    );
